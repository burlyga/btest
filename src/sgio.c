	 ...
	 void *data, unsigned int data_bytes, unsigned int timeout_secs)
{
	...
	DEBUG3("SG_IO: ATA_%u status=0x%x, host_status=0x%x, driver_status=0x%x",
	...
	if (io_hdr.host_status || io_hdr.driver_status != SG_DRIVER_SENSE
		...
		WARN("SG_IO: bad response (not CHECK_CONDITION)");
		errno = EBADE;
		return -1;
	}
	...
	DEBUG3("      ATA_%u stat=%02x err=%02x nsect=%02x lbal=%02x lbam=%02x lbah=%02x dev=%02x",
	...
	if (tf->status & (ATA_STAT_ERR | ATA_STAT_DRQ)) {
		WARN("I/O error, ata_op=0x%02x ata_status=0x%02x ata_error=0x%02x",
...
int do_taskfile_cmd(int fd, struct hdio_taskfile *r, unsigned int timeout_secs)
{
	...
	if (conf.debug) {
		...
		if (r->oflags.lob.feat)
			printf(" feat");
		if (r->oflags.lob.nsect)
			printf(" nsect");
		if (r->oflags.lob.lbal)
			printf(" lbal");
		if (r->oflags.lob.lbam)
			printf(" lbam");
		if (r->oflags.lob.lbah)
			printf(" lbah");
		if (r->oflags.lob.dev)
			printf(" dev");
		if (r->oflags.lob.command)
			printf(" command");
		printf(" }\n");
		printf("oflags.hob_all=0x%02x, flags={", r->oflags.hob_all);
		if (r->oflags.hob.feat)
			printf(" feat");
		if (r->oflags.hob.nsect)
			printf(" nsect");
		if (r->oflags.hob.lbal)
			printf(" lbal");
		if (r->oflags.hob.lbam)
			printf(" lbam");
	...
	if (rc == 0 || errno == EIO) {
		if (r->iflags.lob.feat)
			r->lob.feat = tf.error;
		if (r->iflags.lob.lbal)
			r->lob.lbal = tf.lob.lbal;
		if (r->iflags.lob.nsect)
			r->lob.nsect = tf.lob.nsect;
		if (r->iflags.lob.lbam)
			r->lob.lbam = tf.lob.lbam;
		if (r->iflags.lob.lbah)
			r->lob.lbah = tf.lob.lbah;
		if (r->iflags.lob.dev)
			r->lob.dev = tf.dev;
		if (r->iflags.lob.command)
			r->lob.command = tf.status;
		if (r->iflags.hob.feat)
			r->hob.feat = tf.hob.feat;
		if (r->iflags.hob.lbal)
			r->hob.lbal = tf.hob.lbal;
		if (r->iflags.hob.nsect)
			r->hob.nsect = tf.hob.nsect;
		if (r->iflags.hob.lbam)
			r->hob.lbam = tf.hob.lbam;
		if (r->iflags.hob.lbah)
	...
	if (conf.debug) {
		...
		if (r->iflags.lob.feat)
			fprintf(stderr, " er=%02x", r->lob.feat);
		if (r->iflags.lob.nsect)
			fprintf(stderr, " ns=%02x", r->lob.nsect);
		if (r->iflags.lob.lbal)
			fprintf(stderr, " ll=%02x", r->lob.lbal);
		if (r->iflags.lob.lbam)
			fprintf(stderr, " lm=%02x", r->lob.lbam);
		if (r->iflags.lob.lbah)
			fprintf(stderr, " lh=%02x", r->lob.lbah);
		if (r->iflags.lob.dev)
			fprintf(stderr, " dh=%02x", r->lob.dev);
		if (r->iflags.lob.command)
			fprintf(stderr, " st=%02x", r->lob.command);
		if (r->iflags.hob.feat)
			fprintf(stderr, " err=%02x", r->hob.feat);
		if (r->iflags.hob.nsect)
			fprintf(stderr, " err=%02x", r->hob.nsect);
		if (r->iflags.hob.lbal)
			fprintf(stderr, " err=%02x", r->hob.lbal);
		if (r->iflags.hob.lbam)
			fprintf(stderr, " err=%02x", r->hob.lbam);
		if (r->iflags.hob.lbah)
			fprintf(stderr, " err=%02x", r->hob.lbah);
		fprintf(stderr, "\n");
		errno = err;
	}
...
void init_hdio_taskfile(struct hdio_taskfile *r, __u8 ata_op, int rw,
			int force_lba48, __u64 lba, unsigned int nsect, int data_bytes)
{
